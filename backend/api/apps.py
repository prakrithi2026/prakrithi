            ...
            if config_obj:
                ...
                if 'hero' in config and 'mobileImages' in config['hero'] and isinstance(config['hero']['mobileImages'], list):
                    ...
                    for img in config['hero']['mobileImages']:
                        if img and img.startswith("data:image/") and len(img) > 600000:
                            optimized = compress_base64_image(img, max_width=1080, max_height=1920, quality=90)
                            if len(optimized) < len(img):
                                new_mobile_images.append(optimized)
                                changed = True
                            else:
                                new_mobile_images.append(img)
                        else:
                            new_mobile_images.append(img)
                    config['hero']['mobileImages'] = new_mobile_images
                # hero productImages
